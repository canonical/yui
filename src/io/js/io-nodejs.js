        ...
        send: function (transaction, uri, config) {
            ...
            if (config.data) {
                ...
                if (rconf.body && rconf.method === "GET") {
                    rconf.uri +=
                        (rconf.uri.indexOf("?") > -1 ? "&" : "?") + rconf.body;
                    rconf.body = "";
                }
            }
            if (config.headers) {
            ...
            Y.log(
                "Initiating " + rconf.method + " request to: " + rconf.uri,
                "info",
                "io"
            );
            Y.IO.request(rconf, function (err, data) {
                ...
                if (err) {
                    ...
                    transaction.c = err;
                    config.notify(
                        err.code === "ETIMEDOUT" ? "timeout" : "failure",
                        transaction,
                        config
                    );
                    return;
                ...
                config.notify(
                    data && data.statusCode >= 200 && data.statusCode <= 299
                        ? "success"
                        : "failure",
                    transaction,
                    config
